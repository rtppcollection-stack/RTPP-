...
const localeFiles = import.meta.glob<{ default: Record<string, any> }>("../locales/*.json", { eager: true });
...
for (const path in localeFiles) {
  ...
  if (match) {
    ...
    LOCALES[langCode] = localeFiles[path].default || localeFiles[path];
  }
}
  ...
  const value = useMemo<I18nCtx>(
    () => ({
      ...
      t: (k: string) => {
        ...
        const legacyVal = TRANSLATIONS[lang]?.[k as any] ?? TRANSLATIONS.en?.[k as any];
